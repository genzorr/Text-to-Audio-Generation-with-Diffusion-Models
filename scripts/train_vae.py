...
class AudioDiffusion(Dataset):
    def __init__(self, data_folder):
        ...
        for file_name in file_names:
            ...
            log_S = librosa.power_to_db(S, ref=np.max, top_db=self.mel.top_db)
            bytedata = (
                ((log_S + self.mel.top_db) * 255 / self.mel.top_db).clip(0, 255) + 0.5
            ).astype(np.uint8)

            image = bytedata.reshape((bytedata.shape[0], bytedata.shape[1], 1))
            image = image[: self.mel.x_res, : self.mel.y_res, :]
            image = (image / 255) * 2 - 1
...
class AudioDiffusionDataModule(pl.LightningDataModule):
    def __init__(self, data_folder, batch_size):
        ...
        self.dataset = AudioDiffusion(data_folder)
        self.num_workers = 4
...
class ImageLogger(Callback):
    def __init__(self, every=1000, hop_length=512, sample_rate=16000, n_fft=2048):
        super().__init__()
        self.every = every
    ...
    def log_images_and_audios(self, pl_module, batch):
        ...
        for k in images:
            ...
            images[k] = (
                (images[k].numpy() * 255).round().astype("uint8").transpose(0, 2, 3, 1)
            )
            for _, image in enumerate(images[k]):
                audio = mel_.image_to_audio(
                ...
                pl_module.logger.experiment.add_audio(
                    ...
                    sample_rate=mel_.get_sample_rate(),
                )
...
class HFModelCheckpoint(ModelCheckpoint):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        convert_ldm_to_hf_vae(
            ldm_checkpoint, self.ldm_config, self.hf_checkpoint, self.sample_size
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--ldm_config_file", type=str, default="config/ldm_autoencoder_kl.yaml"
    )
    parser.add_argument(
        "--ldm_checkpoint_dir", type=str, default="models/ldm-autoencoder-kl"
    )
    parser.add_argument(
        "--hf_checkpoint_dir", type=str, default="models/autoencoder-kl"
    )
